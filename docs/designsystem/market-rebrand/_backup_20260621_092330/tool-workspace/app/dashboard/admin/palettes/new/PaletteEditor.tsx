...
const DEFAULTS = {
  ...
  'primary':       '#3D5689',
  'primary-hover': '#46639B',
  'primary-light': '#EBEFF6',
  'primary-pale':  '#EBEFF6',
  'accent':        '#2E7D57',
  'accent-light':  '#EAF3EE',
  'green':         '#2E7D57',
...
export default function PaletteEditor({ userId }: { userId: string }) {
  ...
  async function handleSave() {
    ...
    const { error: err } = await supabase.from('palettes').insert({
      ...
      tokens:      { color: colors, font: { base: "'Archivo', system-ui, -apple-system, sans-serif", display: "'Archivo Black', sans-serif", mono: "'Roboto Mono', monospace", scale: '1rem' }, radius: { sm: '7px', md: '9px', lg: '14px', xl: '16px', '2xl': '24px', pill: '100px' }, shadow: { card: '0 8px 24px rgba(0,0,0,.08)', hover: '0 12px 30px rgba(0,0,0,.12)', focus: '0 0 0 3px rgba(61,86,137,.35)' } },
      created_by:  userId,
    })
