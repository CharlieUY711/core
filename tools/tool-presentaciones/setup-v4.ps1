# CORE Presentaciones — Setup V4 (centrado correcto, estilos inline)
# cd C:\CORE\apps\core-presentaciones
# Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass
# .\setup-v4.ps1

$base = Split-Path -Parent $MyInvocation.MyCommand.Path
Write-Host "CORE Presentaciones V4" -ForegroundColor Cyan
...
const POS = [{x:50,y:50},{x:50,y:15},{x:82,y:32},{x:82,y:68},{x:50,y:85},{x:18,y:68},{x:18,y:32}];
const FROM = [{x:50,y:50},{x:8,y:5},{x:95,y:12},{x:92,y:88},{x:50,y:98},{x:3,y:82},{x:6,y:18}];
...
export function WhatIfSection() {
  ...
  return (
    <section id="whatif" ref={ref} style={{position:"relative",minHeight:"100vh",display:"flex",alignItems:"center",background:"#0A1F3D",borderTop:"1px solid #1e3354"}}>
      <div style={{width:"100%",maxWidth:"1280px",margin:"0 auto",padding:"128px 24px",
        display:"grid",gridTemplateColumns:"1fr 1fr",gap:"64px",alignItems:"center"}}>
        {/* Visual */}
        <motion.div initial={{opacity:0}} animate={inView?{opacity:1}:{}}
          style={{aspectRatio:"1",width:"100%",maxWidth:"480px",margin:"0 auto"}}>
          <svg viewBox="0 0 100 100" style={{width:"100%",height:"100%"}}>
            {POS.slice(1).map((p,i) => (
              <motion.line key={i} x1={p.x} y1={p.y} x2={50} y2={50}
                stroke="rgba(59,130,246,0.2)" strokeWidth="0.4"
                initial={{opacity:0}} animate={inView?{opacity:1}:{}} transition={{delay:0.8+i*0.1}} />
            ))}
            {t.whatif.primitives.map((label,i) => {
              const f=FROM[i+1], to=POS[i+1];
              return (
                <motion.g key={label}
                  initial={{x:f.x-to.x,y:f.y-to.y,opacity:0}}
                  animate={inView?{x:0,y:0,opacity:1}:{}}
                  transition={{delay:0.2+i*0.08,duration:0.9,ease:[0.16,1,0.3,1]}}>
                  <circle cx={to.x} cy={to.y} r={5} fill="rgba(15,56,117,0.6)" stroke="#1e3354" strokeWidth="0.6"/>
                  <text x={to.x} y={to.y+0.5} textAnchor="middle" dominantBaseline="middle"
                    fontSize="3" fill="#8fa3bf" fontFamily="system-ui">{label}</text>
                </motion.g>
              );
            })}
            <motion.g initial={{opacity:0,scale:0}} animate={inView?{opacity:1,scale:1}:{}} transition={{delay:0.6}}>
              ...
              <circle cx={50} cy={50} r={8} fill="rgba(27,90,196,0.3)"/>
        ...
        <div>
          ...
          <motion.h2 initial={{opacity:0,y:24}} animate={inView?{opacity:1,y:0}:{}} transition={{delay:0.3}}
            style={{fontWeight:700,fontSize:"clamp(28px,4vw,52px)",lineHeight:1.15,marginBottom:"24px",
          ...
          <motion.p initial={{opacity:0}} animate={inView?{opacity:1}:{}} transition={{delay:0.5}}
            style={{color:"#8fa3bf",fontSize:"clamp(14px,1.5vw,18px)",lineHeight:1.7,marginBottom:"32px",fontWeight:300}}>
            {t.whatif.sub}
          </motion.p>
          <div style={{display:"flex",flexDirection:"column",gap:"10px"}}>
            {t.whatif.primitives.map((p,i) => (
              <motion.div key={p} initial={{opacity:0,x:-16}} animate={inView?{opacity:1,x:0}:{}}
...
export function DifferenceSection() {
  ...
  return (
    <section id="difference" ref={ref} style={{position:"relative",minHeight:"100vh",display:"flex",alignItems:"center",background:"#0A1F3D",borderTop:"1px solid #1e3354"}}>
      <div style={{width:"100%",maxWidth:"1280px",margin:"0 auto",padding:"128px 24px"}}>
        <motion.p initial={{opacity:0}} animate={inView?{opacity:1}:{}}
          style={{fontFamily:"monospace",fontSize:"11px",color:"#8fa3bf",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:"64px"}}>
          {t.difference.label}
        </motion.p>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"96px",marginBottom:"96px"}}>
          <div>
            <motion.p initial={{opacity:0}} animate={inView?{opacity:1}:{}} transition={{delay:0.2}}
              style={{fontFamily:"monospace",fontSize:"11px",color:"#4a6080",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:"32px"}}>
              {t.difference.before_label}
            </motion.p>
            {t.difference.before.map((item,i) => (
              <motion.p key={item} initial={{opacity:0,x:-20}} animate={inView?{opacity:1,x:0}:{}}
                transition={{delay:0.3+i*0.1}}
                style={{fontWeight:700,fontSize:"clamp(24px,3vw,40px)",color:"#4a6080",
          ...
          <div>
            <motion.p initial={{opacity:0}} animate={inView?{opacity:1}:{}} transition={{delay:0.6}}
              style={{fontFamily:"monospace",fontSize:"11px",color:"#8fa3bf",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:"32px"}}>
              {t.difference.after_label}
            </motion.p>
            {t.difference.after.map((item,i) => (
              <motion.p key={item} initial={{opacity:0,x:20}} animate={inView?{opacity:1,x:0}:{}}
                transition={{delay:0.7+i*0.1}}
                style={{fontWeight:700,fontSize:"clamp(24px,3vw,40px)",lineHeight:1.2,marginBottom:"16px",
                  background:"linear-gradient(135deg,#fff 30%,#f5c870 100%)",
                  WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
                {item}
              </motion.p>
        ...
        <motion.div initial={{opacity:0,y:24}} animate={inView?{opacity:1,y:0}:{}} transition={{delay:1.4}}
          style={{paddingTop:"48px",borderTop:"1px solid #1e3354"}}>
          <p style={{color:"#4a6080",fontSize:"clamp(16px,1.8vw,22px)",marginBottom:"8px",
...
Write-Host ""
Write-Host "Setup V4 listo. pnpm dev" -ForegroundColor Yellow