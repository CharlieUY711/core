# CORE Presentaciones — Setup V2 (paleta core-landing)
# Ejecutar desde: C:\CORE\apps\core-presentaciones
# Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass

$base = Split-Path -Parent $MyInvocation.MyCommand.Path
Write-Host "CORE Presentaciones — Setup V2" -ForegroundColor Cyan
...
Write-Host "OK  Carpetas" -ForegroundColor Green
...
export function ExperienceNav() {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
      scrolled
        ? "py-3 bg-[#0a1f3d]/90 backdrop-blur border-b border-[#1e3354]"
        : "py-5 bg-transparent"
    }`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
            ...
            {LOCALES.map((l, i) => (
              <span key={l.value} className="flex items-center gap-1">
                <button
                  ...
                  className={`text-xs font-medium tracking-widest uppercase px-1 py-0.5 transition-colors ${
                    locale === l.value ? "text-white" : "text-[#8fa3bf] hover:text-white"
                  }`}
                >
                  {l.label}
                </button>
                {i < LOCALES.length - 1 && (
                  <span className="text-[#1e3354] text-xs">·</span>
                )}
              </span>
            ))}
          </div>
          <a
            href="#vision"
            className="px-4 py-2 rounded-lg bg-[#1b5ac4] hover:bg-[#3b82f6] text-white text-sm font-medium transition-all"
          >
            {t.nav.cta}
          </a>
        </div>
...
export function HeroSection() {
  ...
  return (
    <section id="hero" className="relative min-h-screen flex items-center justify-center overflow-hidden"
      style={{ background: "radial-gradient(ellipse 80% 60% at 50% -10%, rgba(27,90,196,0.35) 0%, transparent 70%), #0A1F3D" }}>

      {/* Grid dots background */}
      <div className="absolute inset-0 opacity-30"
        style={{ backgroundImage: "radial-gradient(circle, rgba(59,130,246,0.15) 1px, transparent 1px)", backgroundSize: "28px 28px" }} />

      {/* Background network subtle */}
      <div className="absolute inset-0 opacity-10">
        <NetworkGraph chaos={0.1} labels={["","","","","","","","",""]} animated />
      </div>
      ...
      <div className="relative z-10 text-center px-8 max-w-4xl mx-auto">
        ...
        >
          <div className="flex items-center justify-center gap-2 mb-8">
            ...
            <span className="text-[#8fa3bf] font-mono text-xs tracking-[0.15em] uppercase">
          ...
          <h1 className="font-bold leading-tight mb-6"
            ...
            <span style={{ background: "linear-gradient(135deg,#fff 30%,#f5c870 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
              CORE
            </span>
          </h1>
        </motion.div>
        ...
        >
          <p className="text-[#8fa3bf] text-xl mb-3" style={{ letterSpacing: "0.02em" }}>
            {t.hero.tagline}
          </p>
          <p className="text-[#4a6080] text-sm tracking-widest">
            {t.hero.sub}
          </p>
        </motion.div>
      </div>
      ...
      >
        <span className="text-[#4a6080] text-xs tracking-[0.15em] uppercase">{t.hero.scroll}</span>
        <motion.div
          animate={{ y: [0, 6, 0] }}
          transition={{ duration: 1.5, repeat: Infinity }}
          className="w-px h-8"
          style={{ background: "linear-gradient(to bottom, #4a6080, transparent)" }}
        />
      </motion.div>
    </section>
  );
...
export function FragmentationSection() {
  ...
  return (
    <section id="fragmentation" ref={ref} className="relative min-h-screen flex items-center"
      style={{ background: "#0A1F3D" }}>
      <div className="w-full max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-32">
        <div>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            ...
            style={{ fontSize: "clamp(28px, 4vw, 52px)", color: "#8fa3bf" }}
          >
          ...
          <div className="flex flex-wrap gap-2 mb-12">
            {t.fragmentation.systems.map((sys, i) => (
              <motion.span
                ...
                className="px-3 py-1.5 text-xs font-mono rounded-lg border tracking-widest uppercase"
                style={{ border: "1px solid #1e3354", color: "#8fa3bf", background: "rgba(15,56,117,0.12)" }}
              >
                {sys}
              </motion.span>
            ))}
          ...
          >
            <p className="font-bold mb-2" style={{ fontSize: "clamp(20px, 2.5vw, 32px)", color: "#8fa3bf" }}>
              {t.fragmentation.problem_a}
            </p>
            <p className="font-bold" style={{ fontSize: "clamp(20px, 2.5vw, 32px)", color: "#fff",
...
const POSITIONS = [
  { x: 50, y: 50 }, { x: 50, y: 15 }, { x: 82, y: 32 },
  { x: 82, y: 68 }, { x: 50, y: 85 }, { x: 18, y: 68 }, { x: 18, y: 32 },
];
const FROM = [
  { x: 50, y: 50 }, { x: 8, y: 5 }, { x: 95, y: 12 },
  { x: 92, y: 88 }, { x: 50, y: 98 }, { x: 3, y: 82 }, { x: 6, y: 18 },
];
...
export function WhatIfSection() {
  ...
  return (
    <section id="whatif" ref={ref} className="relative min-h-screen flex items-center"
      style={{ background: "#0A1F3D" }}>
      <div className="absolute top-0 left-0 right-0 h-px" style={{ background: "linear-gradient(to right, transparent, #1e3354, transparent)" }} />
      ...
      <div className="w-full max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center py-32">
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          ...
          <svg viewBox="0 0 100 100" className="w-full h-full">
            {POSITIONS.slice(1).map((pos, i) => (
              <motion.line key={i} x1={pos.x} y1={pos.y} x2={50} y2={50}
                stroke="rgba(59,130,246,0.2)" strokeWidth="0.4"
                initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
                transition={{ delay: 0.8 + i * 0.1 }} />
            ))}
            {t.whatif.primitives.map((label, i) => {
              const from = FROM[i + 1]; const to = POSITIONS[i + 1];
              return (
                <motion.g key={label}
                  initial={{ x: from.x - to.x, y: from.y - to.y, opacity: 0 }}
                  animate={inView ? { x: 0, y: 0, opacity: 1 } : {}}
                  transition={{ delay: 0.2 + i * 0.08, duration: 0.9, ease: [0.16, 1, 0.3, 1] }}>
                  <circle cx={to.x} cy={to.y} r={4.5} fill="rgba(15,56,117,0.6)"
                    stroke="#1e3354" strokeWidth="0.6" />
                  <text x={to.x} y={to.y + 0.5} textAnchor="middle" dominantBaseline="middle"
                    fontSize="3" fill="#8fa3bf" fontFamily="system-ui, sans-serif">{label}</text>
                </motion.g>
              );
            })}
            <motion.g initial={{ opacity: 0, scale: 0 }} animate={inView ? { opacity: 1, scale: 1 } : {}}
              transition={{ delay: 0.6, duration: 0.6 }}>
              <circle cx={50} cy={50} r={12} fill="rgba(27,90,196,0.15)" />
              <circle cx={50} cy={50} r={8} fill="rgba(27,90,196,0.25)" />
              <circle cx={50} cy={50} r={5} fill="#1b5ac4" />
              <text x={50} y={50.5} textAnchor="middle" dominantBaseline="middle"
                fontSize="2.8" fill="white" fontFamily="system-ui" fontWeight="600">CORE</text>
            </motion.g>
          </svg>
        </motion.div>
        ...
        <div className="order-1 lg:order-2">
          <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
            className="font-mono text-xs text-[#8fa3bf] tracking-[0.15em] uppercase mb-4">
            {t.whatif.question}
          </motion.p>
          <motion.h2 initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            style={{ fontSize: "clamp(28px, 4vw, 52px)",
              background: "linear-gradient(135deg,#fff 30%,#7db8f7 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
            {t.whatif.question}
          </motion.h2>
          <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
            transition={{ delay: 0.5 }} className="text-[#8fa3bf] text-lg leading-relaxed mb-8 font-light">
            {t.whatif.sub}
          </motion.p>
          <div className="flex flex-col gap-2">
            {t.whatif.primitives.map((p, i) => (
              <motion.div key={p} initial={{ opacity: 0, x: -16 }}
                animate={inView ? { opacity: 1, x: 0 } : {}}
                transition={{ delay: 0.7 + i * 0.07 }}
                className="flex items-center gap-3">
                <span className="text-[#c9993a] text-sm">◆</span>
                <span className="text-[#8fa3bf] text-sm">{p}</span>
              </motion.div>
            ))}
          </div>
        </div>
...
export function DifferenceSection() {
  ...
  return (
    <section id="difference" ref={ref} className="relative min-h-screen flex items-center"
      style={{ background: "#0A1F3D" }}>
      <div className="absolute top-0 left-0 right-0 h-px"
        style={{ background: "linear-gradient(to right, transparent, #1e3354, transparent)" }} />

      <div className="w-full max-w-7xl mx-auto px-6 py-32">
        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          className="font-mono text-xs text-[#8fa3bf] tracking-[0.15em] uppercase mb-16">
          {t.difference.label}
        </motion.p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24">
          ...
          <div>
            <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
              ...
              className="font-mono text-xs text-[#4a6080] tracking-[0.15em] uppercase mb-8">
            ...
            <div className="flex flex-col gap-5">
              {t.difference.before.map((item, i) => (
                <motion.p key={item}
                  ...
                  className="font-bold"
                  style={{ fontSize: "clamp(24px, 3vw, 40px)", color: "#4a6080",
                    textDecoration: "line-through", textDecorationColor: "rgba(74,96,128,0.4)" }}>
                  {item}
                </motion.p>
              ))}
            </div>
          ...
          <div>
            <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
              ...
              className="font-mono text-xs text-[#8fa3bf] tracking-[0.15em] uppercase mb-8">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          className="mt-24 pt-12 border-t border-[#1e3354]">
          <p className="text-[#4a6080] text-lg mb-2"
            style={{ textDecoration: "line-through", textDecorationColor: "rgba(74,96,128,0.3)" }}>
            {t.claim.before}
          </p>
          <p className="font-bold" style={{ fontSize: "clamp(20px, 2.5vw, 36px)",
...
export function GlobalSection() {
  ...
  return (
    <section id="global" ref={ref} className="relative" style={{ background: "#0A1F3D" }}>
      <div className="absolute top-0 left-0 right-0 h-px"
        style={{ background: "linear-gradient(to right, transparent, #1e3354, transparent)" }} />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-24 pb-8">
        ...
        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.2 }} className="font-bold leading-tight mb-4"
          style={{ fontSize: "clamp(28px, 4vw, 52px)",
        ...
        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          transition={{ delay: 0.4 }} className="text-[#8fa3bf] text-lg font-light max-w-xl">
          {t.global.sub}
        </motion.p>
      </div>
      ...
      {/* Globe full width */}
      <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
        transition={{ duration: 1, delay: 0.3 }} className="relative w-full">
        <div className="absolute top-0 left-0 right-0 z-10 pointer-events-none"
          style={{ height: "100px", background: "linear-gradient(to bottom, #0A1F3D 0%, transparent 100%)" }} />
        <CoreGlobe activeLayer="all" autoRotate={false} className="w-full" />
        <div className="absolute bottom-0 left-0 right-0 z-10 pointer-events-none"
          style={{ height: "100px", background: "linear-gradient(to top, #0A1F3D 0%, transparent 100%)" }} />
      </motion.div>

      {/* Regions */}
      <div className="relative z-10 max-w-7xl mx-auto px-6 pb-16 pt-2">
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          transition={{ delay: 0.8 }} className="flex items-center gap-6 flex-wrap">
          {t.global.regions.map((region, i) => (
            <span key={region} className="flex items-center gap-6">
              <span className="font-mono text-xs text-[#8fa3bf] tracking-widest uppercase">{region}</span>
              {i < t.global.regions.length - 1 && <span className="text-[#1e3354] text-xs">→</span>}
            </span>
          ))}
          <span className="text-[#1e3354] text-xs">→</span>
          <span className="font-mono text-xs text-white tracking-widest uppercase font-bold">CORE</span>
          <span className="text-[#1e3354] text-xs">→</span>
          <span className="font-mono text-xs text-[#8fa3bf] tracking-widest uppercase">LATAM</span>
        </motion.div>
      </div>
    </section>
  );
...
function CounterStat({ target, suffix, label, delay, inView }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
      ...
      className="rounded-2xl p-6 border border-[#1e3354] bg-[rgba(15,56,117,0.12)] hover:bg-[rgba(27,90,196,0.22)] hover:border-blue-500/30 transition-all duration-300">
      <p className="font-bold leading-none mb-2"
        style={{ fontSize: "clamp(36px, 4vw, 56px)",
          background: "linear-gradient(135deg,#fff 30%,#f5c870 100%)",
          WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
        {target >= 1000 ? value.toLocaleString() : value}
        {suffix && <span style={{ color: "#f5c870" }}>{suffix}</span>}
      </p>
      <p className="text-[#8fa3bf] font-mono text-xs tracking-widest uppercase">{label}</p>
    </motion.div>
  );
}
...
export function VisionSection() {
  ...
  return (
    <section id="vision" ref={ref} className="relative min-h-screen flex items-center"
      style={{ background: "#0A1F3D" }}>
      <div className="absolute top-0 left-0 right-0 h-px"
        style={{ background: "linear-gradient(to right, transparent, #1e3354, transparent)" }} />
      ...
      <div className="w-full max-w-7xl mx-auto px-6 py-32">
        <motion.p initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          className="font-mono text-xs text-[#8fa3bf] tracking-[0.15em] uppercase mb-6">
          {t.vision.label}
        </motion.p>

        <motion.h2 initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 0.2 }} className="font-bold mb-16"
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-20">
          {t.vision.stats.map((stat, i) => (
            <CounterStat key={stat.label} target={stat.value} suffix={stat.suffix}
              label={stat.label} delay={0.2 + i * 0.15} inView={inView} />
          ))}
        </div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          transition={{ delay: 1 }} className="mb-16">
          <p className="font-bold text-[#8fa3bf] mb-2"
        ...
        <motion.div initial={{ opacity: 0 }} animate={inView ? { opacity: 1 } : {}}
          transition={{ delay: 1.4 }}
          className="pt-12 border-t border-[#1e3354] mb-12">
          <p className="text-[#4a6080] text-lg mb-2"
            style={{ textDecoration: "line-through", textDecorationColor: "rgba(74,96,128,0.3)" }}>
            {t.claim.before}
          </p>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={inView ? { opacity: 1, y: 0 } : {}}
          ...
          <a href="mailto:hello@core.com"
            className="inline-block px-8 py-3.5 rounded-xl bg-[#1b5ac4] hover:bg-[#3b82f6] text-white font-medium text-sm transition-all hover:shadow-xl hover:shadow-blue-500/30 hover:-translate-y-0.5">
            {t.vision.cta} →
          </a>
        </motion.div>
      </div>
...
export function CoreGlobe({
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      R = Math.min(W, H) * 0.32; cx = W / 2; cy = H / 2;
    }
    resize();
  ...
  const getHubAt = useCallback((clientX: number, clientY: number): Hub | null => {
    ...
    const R = Math.min(W,H)*0.32, cx = W/2, cy = H/2;
    const { x:rX, y:rY } = rotRef.current;
    for (const h of hubs) {
...
Write-Host ""
Write-Host "Listo. Copiá world-110m.json a public\ y ejecutá: pnpm dev" -ForegroundColor Yellow