...
export default async function AdminPage() {
  ...
  const [{ data: palettes }, { data: designs }, { data: portals }] = await Promise.all([
    ...
    supabase.from('portals').select('id, name_es, status, access, palette_id, design_id, features, vercel_project_id, vercel_project_name, repo_url, repo_branch, repo_url').order('order_index'),
  ])
