...
async function getFileSha(repo: string, branch: string, filePath: string): Promise<string | null> {
  const res = await ghApi(`/repos/${GITHUB_OWNER}/${repo}/contents/${filePath}?ref=${branch}`, 'GET')
...
async function getTreePaths(repo: string, branch: string): Promise<string[]> {
  const res = await ghApi(`/repos/${GITHUB_OWNER}/${repo}/git/trees/${branch}?recursive=1`, 'GET')
  if (!res.ok) return []
  const data = await res.json()
  return (data.tree as { path: string; type: string }[]).filter(f => f.type === 'blob').map(f => f.path)
}

async function deleteFile(repo: string, branch: string, filePath: string, sha: string, message: string) {
  return ghApi(`/repos/${GITHUB_OWNER}/${repo}/contents/${filePath}`, 'DELETE', { message, sha, branch })
}
...
async function upsertFile(repo: string, branch: string, filePath: string, content: string, message: string) {
  ...
  return ghApi(`/repos/${GITHUB_OWNER}/${repo}/contents/${filePath}`, 'PUT', {
    message, content: Buffer.from(content).toString('base64'), branch, ...(sha ? { sha } : {}),
  })
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const mwRes = await upsertFile(repo, branch, 'middleware.ts', MIDDLEWARE_ESTANDAR,
      'chore(ws): reemplaza middleware propio por estándar del ecosistema')
    if (mwRes.ok) { log.push('✓ middleware.ts estándar commiteado') }
    else { const err = await mwRes.json(); errors.push(`✕ middleware.ts: ${err.message || mwRes.status}`) }
    ...
    for (const dp of DP_TO_REMOVE) {
      if (dp.isDir) {
        ...
        for (const filePath of dirFiles) {
          const sha = await getFileSha(repo, branch, filePath)
          if (!sha) continue
          const res = await deleteFile(repo, branch, filePath, sha, `chore(ws): elimina DP — ${filePath}`)
          if (res.ok) { log.push(`✓ eliminado ${filePath}`) }
          else { errors.push(`✕ ${filePath}: no se pudo eliminar`) }
        }
      } else {
        if (!allFiles.includes(dp.path)) continue
        const sha = await getFileSha(repo, branch, dp.path)
        if (!sha) continue
        const res = await deleteFile(repo, branch, dp.path, sha, `chore(ws): elimina DP — ${dp.path}`)
        if (res.ok) { log.push(`✓ eliminado ${dp.path}`) }
        else { errors.push(`✕ ${dp.path}: no se pudo eliminar`) }
      }
    }
