...
import {
  User, Zap, CalendarDays, FileText, GitBranch,
  ChevronDown, Building2, MapPin, Briefcase, Users,
} from "lucide-react";
...
const ACTIVITY_DOT: Record<string, string> = {
  high:   "bg-emerald-500 animate-pulse",
  medium: "bg-amber-500",
  low:    "bg-slate-400",
};
...
const FALLBACK_ACT = ACTIVITY_LABEL["low"];
...
export function RightPanel({
  ...
  useEffect(() => {
    if (selectedCompanyId !== displayCompanyId) {
      fadeOut(() => setDisplayCompanyId(selectedCompanyId));
    }
  }, [selectedCompanyId]);

  // ── Guard: si no hay empresas no renderizamos nada ───────────────
  if (companies.length === 0) return null;

  const company = companies.find((c) => c.id === displayCompanyId) ?? companies[0];

  // Doble guard por si el find devuelve undefined en un edge case
  ...
  const act            = ACTIVITY_LABEL[company.activity] ?? FALLBACK_ACT;
  ...
  return (
    <div className="flex flex-col h-full bg-white overflow-hidden">
      ...
      <div className="shrink-0 bg-white border-b border-slate-200">
        ...
        {/* Fila superior: título + selector de empresa */}
        <div className="px-5 pt-4 pb-3 flex items-start justify-between gap-4">
          ...
          <div className="relative shrink-0">
            ...
            >
              <div className={`w-2 h-2 rounded-full shrink-0 ${ACTIVITY_DOT[company.activity] ?? "bg-slate-400"}`} />
            ...
            {showCompanyMenu && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 z-40 bg-white border border-slate-200 rounded-2xl shadow-xl min-w-[260px] overflow-hidden">
                  ...
                  <div className="py-1">
                    {companies.map((c) => {
                      const a = ACTIVITY_LABEL[c.activity] ?? FALLBACK_ACT;
                      return (
                        <button
                          key={c.id}
        ...
        {/* Tabs */}
        <div className="px-5 flex items-center gap-0.5 overflow-x-auto">
          {tabs.map((tab) => {
            const Icon = tab.icon;
