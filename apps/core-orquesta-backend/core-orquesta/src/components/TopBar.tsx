import { Activity, Settings, KeyRound, Layers, Bell, AlertTriangle, LogOut } from "lucide-react";
...
export function TopBar({
  ...
  activeMotors, totalMotors, errorMotors, userEmail, onSignOut,
}: TopBarProps) {
  return (
        ...
        {userEmail && onSignOut && (
          <>
            ...
            <button onClick={onSignOut} title={`Cerrar sesión (${userEmail})`}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-slate-500 hover:text-red-300 hover:bg-white/5 rounded-lg transition-all text-[11px] border border-transparent hover:border-white/10">
              <LogOut className="w-3.5 h-3.5" />
              <span className="hidden md:block max-w-[120px] truncate">{userEmail}</span>
            </button>
          </>
        )}
      </div>
