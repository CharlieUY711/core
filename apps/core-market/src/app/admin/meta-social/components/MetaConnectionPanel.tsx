...
export function MetaConnectionPanel({
  platform, status, error, isConfigured, onReconnect, children,
        ...
        {!isConfigured && (
          <div style={{
            ...
            {/* SIN BOTÓN, a propósito.
                Es UNA conexión: un login de Facebook resuelve las tres. Tres
                botones iguales hacían dudar de si había que apretar los tres.
                "Conectar" está en la barra, que es donde están las acciones en
                todo el panel.

                Lo que sí es distinto en cada una es QUÉ hace falta del lado de
                Meta, y eso es lo único que una tarjeta de estado tiene que
                decir: por qué no está y qué falta. */}
            <strong style={{ color: T.textDark }}>Sin conectar.</strong>{' '}
            {platform === 'Instagram' &&
              'Necesita una cuenta Business o Creator vinculada a una página de Facebook. ' +
              'Si es una cuenta personal, no aparece.'}
            {platform === 'Facebook' &&
              'Necesita una página de Facebook. Instagram y WhatsApp cuelgan de ella, ' +
              'así que es la primera que tiene que quedar conectada.'}
            {platform === 'WhatsApp' &&
              'Necesita una cuenta de WhatsApp Business asociada a tu negocio en Meta.'}
            {' '}Se conecta desde la barra, arriba.
          </div>
        )}
