-- ===========================================================================
-- La orden congela el desglose de impuesto
-- ===========================================================================
--
-- Los precios se publican con impuestos incluidos, asi que el total no cambia:
-- lo que se agrega es el desglose de cada linea -neto, tasa y monto- guardado
-- en el momento de la venta.
--
-- Se congela por la misma razon que `ordenes.tipo_cambio`: la tasa del
-- articulo puede cambiar manana y la factura ya emitida no. Recalcularla
-- despues daria un numero distinto al que se cobro.
--
-- Y se guarda un ARRAY, no una tasa, aunque hoy siempre sea una linea. Una
-- canasta con vino y turron no tiene una tasa: cada parte lleva la suya y hay
-- que desglosarlas. Cuando eso exista, lo que cambia es lo que devuelve
-- lineas_de_impuesto; esto no se toca.
-- ===========================================================================
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    SELECT b.titulo, b.status::text, coalesce(v.stock, 0), b.id
      INTO v_nombre_prod, v_status, v_stock, v_base_id
    ...
    -- El precio sale de precio_de_canal: el del listing si ese canal tiene uno
    -- propio, el de la variante si no. Es la MISMA funcion que usa la vidriera,
    -- a proposito: si cada lado resolviera por su cuenta, la tienda podria
    -- mostrar un numero y cobrar otro.
    SELECT precio, moneda INTO v_price, v_currency
      FROM precio_de_canal(v_product_id, v_tipo);
    ...
    -- El desglose de impuesto se CONGELA en la orden, igual que el tipo de
    -- cambio: la tasa del articulo puede cambiar manana, la factura no.
    --
    -- Se guarda un array y no una tasa, aunque hoy siempre sea una sola linea.
    -- Un compuesto tipo canasta lleva una por componente -cada parte con su
    -- propia tasa- y ese dia esto no cambia: cambia lo que devuelve
    -- lineas_de_impuesto, y nada mas.
    SELECT coalesce(jsonb_agg(jsonb_build_object(
             'code', l.code, 'name', l.name, 'rate', l.rate,
             'bruto', l.bruto, 'neto', l.neto, 'monto', l.monto
           )), '[]'::jsonb)
      INTO v_impuestos
      FROM lineas_de_impuesto(v_base_id, v_price, v_quantity) l;
