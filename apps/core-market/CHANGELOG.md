...
## 2026-08-22 — DEC-011: API Vault Credential Provider world-class design (design only, nothing implemented)

### Objective
Produce an implementation-ready design for API Vault as the platform's
generic Credential Provider (RESOLVE/DELIVER/REPORT/HEALTH/LIFECYCLE),
per the brief following up on DEC-010. Design phase only — no code, no
migrations, no schema, no secrets.

### Evidence gathered this session (verified by direct read)
`supabase/migrations/20260822001700_api_vault_client_exposed.sql`,
`src/lib/core-apivault/src/services/{apiVaultTypes.ts,
getClientCredential.ts, index.ts}`, `ApiVaultPage.tsx` (partial, targeted
at the `client_exposed`/`type==='token'` gating logic),
`mapboxToken.ts`, `AddressMap.tsx`/`AddressAutocomplete.tsx` (partial),
`AdminProfile.tsx` (existence only), plus the full existing
`.agent/DECISIONS.md`/`HANDOFF.md`/`CHANGELOG.md`.

### Critical evidence gap, stated explicitly
`apiVaultService.ts`, `MLVaultService`, `TokenManager`, `OAuthService`,
`ml-oauth`, `ml-webhook`, `ml-sync`, `publicar-en-ml`, `extract-catalog`,
and the base `api_vault` migration (`20260607_api_vault.sql`) are **not
present** in this session's snapshot — confirmed via an exhaustive file
search. The design's Vault-side contract (RESOLVE/DELIVER/REPORT/HEALTH/
LIFECYCLE, tenant/global resolution, security model, error taxonomy,
concurrency, deliverable/rollback/testing strategy) is grounded in files
actually read and does not depend on ML's internals. The ML migration
section is explicitly marked LOW CONFIDENCE / not implementation-ready,
since it could only be written from secondhand references in prior
`.agent/*` entries, not from reading ML's actual source.

### Changes
- New file: `.agent/DEC-011-api-vault-credential-provider-design.md` —
  the full 30-section design document, including an explicit "evidence
  base" section separating verified-this-session from reported-context.
- `.agent/DECISIONS.md`: added DEC-011, pointing to the design document
  and summarizing the evidence-gap caveat. DEC-009 and DEC-010 untouched.
- `.agent/HANDOFF.md`: added UPDATE 6, with an explicit checklist of what
  the next agent must read before writing any implementation code.

### Files
.agent/DEC-011-api-vault-credential-provider-design.md (new),
.agent/DECISIONS.md, .agent/HANDOFF.md, .agent/CHANGELOG.md

### Verification
Documentation/design only — no build or typecheck run, nothing to
verify against running code.

### Result
DESIGNED / APPROVED — IMPLEMENTATION PENDING. The Vault-side contract is
implementation-ready. ML migration is not — requires the next agent to
read the ML integration files this session could not access before
touching any ML code (including the reported `ml-webhook`
`provider`/`platform` mismatch, which remains unverified).

### Remaining
Implement per the design document's section 26 order. Before any code:
locate and read `apiVaultService.ts`, the base `api_vault` migration, and
the ML integration files. META stays blocked until the Vault contract is
implemented and verified.

## 2026-08-22 — DEC-010: API Vault Credential Provider direction recorded (documentation only, nothing implemented)

### Objective
Record a new architectural direction, approved by the human in this
session: API Vault as the central Credential Provider (generic
RESOLVE/DELIVER/REPORT, persisted health/status, tenant→global
resolution, eventual `ml-webhook` fix), with META as a downstream
consumer. Documentation-only session — no code, no migrations, no
secrets changed.

### Important caveat
This session worked from a partial zip snapshot containing only
`.agent/DECISIONS.md`, `.agent/HANDOFF.md`, and `.agent/CHANGELOG.md` —
no `AGENTS.md`, `CURRENT.md`, `ARCHITECTURE.md`, or `TASK.md` were
present. No evidence of a prior API Vault Credential Provider audit, a
META integration audit, or a confirmed `ml-webhook` bug exists anywhere
in this snapshot (`ml-webhook`'s source isn't even included). DEC-010
is recorded strictly as a newly approved direction, not as a
previously-completed audit — see DEC-010 for the full caveat.

### Changes
- `.agent/DECISIONS.md`: added DEC-010 (PROPOSED / APPROVED NOW, not
  implemented). DEC-009 left untouched — not reinterpreted.
- `.agent/HANDOFF.md`: added UPDATE 5 pointing the next agent at DEC-010
  and flagging that implementation has not started.
- `.agent/CURRENT.md` / `.agent/TASK.md`: **not updated** — not present
  in this session's snapshot.

### Files
.agent/DECISIONS.md, .agent/HANDOFF.md, .agent/CHANGELOG.md

### Verification
Documentation only — no build/typecheck run, nothing to verify.

### Result
DEC-010 exists as an approved-but-unimplemented direction. Next agent
should start with generic `RESOLVE`, and must independently verify the
`ml-webhook` claim by reading the actual file before acting on it.

### Remaining
Full implementation of DEC-010's scope (RESOLVE, DELIVER, REPORT,
health/status persistence, tenant→global, `ml-webhook` fix,
de-duplication of existing resolution mechanisms). META implementation
stays blocked until then. `CURRENT.md`/`TASK.md` should be updated with
this direction once available.
...
## 2026-08-22 — Mapbox token: vault genérico `client_exposed` (fuera de scope de tokens/DEC-007-008)

### Objetivo
Diagnosticar y resolver por qué `AddressMap.tsx` no recibía el token de
Mapbox en local (`Error: An API access token is required to use Mapbox GL`).
Se pidió explícitamente ir más allá del fix trivial (`.env.local`) y que el
API Vault sea la fuente de verdad, de forma genérica (sin nombres de
plataforma hardcodeados), para Mapbox, Supabase o cualquier otro servicio.

### Diagnóstico (fase 1, sin código)
`AddressMap.tsx:14`, `AddressAutocomplete.tsx:12` y
`AdminProfile.tsx:447` leían `import.meta.env.VITE_MAPBOX_TOKEN` de forma
síncrona. Esa clave no existía en `.env.local` ni en `.env.example`. La
credencial "Geoposicionamiento" (Mapbox) sí existía, pero en la tabla
`api_vault` — RLS `auth.uid() = user_id`, sin conexión alguna con las env
vars de Vite. Dos mecanismos desconectados, no un bug de código.

### Cambios
- `supabase/migrations/20260822001700_api_vault_client_exposed.sql` —
  columna `client_exposed boolean default false` en `api_vault` + policy
  RLS **aditiva** (`USING (client_exposed = true)`, se combina con OR con
  la policy existente, no la reemplaza) + índice de soporte. Aplicada
  manualmente por el usuario en Supabase antes de escribir este archivo
  (queda versionada/replayable, no re-aplicada por el agente).
- `src/lib/core-apivault/src/services/getClientCredential.ts` — helper
  genérico: `getClientCredential(supabase, platform, { tenantId? })` lee
  `value` de la fila `client_exposed=true` para esa `platform`
  (`tenant_id IS NULL` = credencial global/fallback, ya contemplado por el
  schema). Sin whitelist de nombres — la seguridad vive en el dato
  (`client_exposed`), no en el código.
- `src/lib/core-apivault/src/services/apiVaultTypes.ts` — agregado
  `client_exposed: boolean` a `ApiVaultEntry`.
- `src/lib/core-apivault/src/services/index.ts` — re-export del helper.
- `src/app/components/maps/mapboxToken.ts` (nuevo) — resolver compartido
  y cacheado: vault primero, fallback a `VITE_MAPBOX_TOKEN` si el vault no
  tiene nada (dev local sigue funcionando sin cambios).
- `src/app/components/maps/AddressMap.tsx` — el `mapboxgl.accessToken =
  import.meta.env...` de module-scope se reemplazó por una resolución
  async dentro del primer `useEffect` (`initMap()` ahora se llama recién
  cuando el token resuelve); cleanup ajustado para cancelar si el
  componente se desmonta antes de que resuelva.
- `src/app/components/maps/AddressAutocomplete.tsx` — `TOKEN` de
  module-scope reemplazado por `await getMapboxToken()` dentro de
  `search()` y `handleGeolocate()`.
- `src/app/admin/pages/AdminProfile.tsx` — mismo cambio en
  `reverseGeocode` de `AddressForm`.
- `src/lib/core-apivault/src/components/ApiVaultPage.tsx` — toggle
  "Exponer al cliente" en el formulario, habilitado **solo** cuando
  `type === 'token'` (no para `api_key/secret/oauth/webhook/...`), con
  advertencia visible; badge "🌐 Expuesta al cliente" en el detalle de la
  entrada.

### Verificación
`npx tsc --noEmit` → solo el warning preexistente de `tsconfig.json`
(`baseUrl` deprecado, sin relación con este cambio). Cero errores nuevos
en los 8 archivos tocados.

### Seguridad — punto explícito
`client_exposed` es un flag genérico, no una whitelist de plataformas.
Cualquier fila marcada así por su dueño queda legible por **cualquier**
cliente (autenticado o anónimo) desde el browser. Es responsabilidad de
quien carga la credencial (reforzada por la UI: el toggle solo aparece
para `type: 'token'`) no marcar así una API secret key, un client secret
de OAuth, o un webhook firmado.

### Pendiente / follow-up
- La migración quedó **aplicada en la DB real por el usuario**, pero
  conviene que alguien corra `supabase migration repair` si el CLI local
  no la reconoce como aplicada (mismo patrón que la sesión de
  `store_id`/JWT claim de más arriba en este changelog).
- No se tocó ningún otro consumidor del vault (ML, MercadoPago, etc.) —
  fuera de scope, ninguna de esas filas debe marcarse `client_exposed`.
- Independiente de DEC-007/DEC-008 (design tokens) — no se tocó ningún
  archivo de esa migración de tokens visuales.
