// ═══════════════════════════════════════════════════════════
// MARKET — Navbar.tsx  (header según spec · Junio 2026)
// Navy de la casa, logo M-squircle, pills Second/Gourmet,
// buscador navy-field, CTA Ingresar en azul madre.
// Misma interfaz y lógica que antes (props, buscador, login,
// carrito, logout, departamentos dinámicos).
// ═══════════════════════════════════════════════════════════
...
const T = {
  navy:        '#0D2B55',
  navyDeep:    '#256647',   // tinte cuando isSH (Second activo)
  navyField:   '#314B6E',
  madre:       '#3D5689',
  madreHover:  '#46639B',
  second:      '#2E7D57',
...
// Color de punto por vertical (departamentos dinámicos)
...
export function Navbar({
  ...
  const navBg = isSH ? T.navyDeep : T.navy;
  ...
  // Pill transversal (Second / Gourmet)
  const Pill = ({ bg, label, onClick, to }:
    { bg: string; label: string; onClick?: () => void; to?: string }) => {
    const inner = (
      <>
        <span style={{ fontFamily: T.black, color: T.white, fontSize: 18, lineHeight: 0.8 }}>M</span>
        <span style={{ fontFamily: T.font, fontWeight: 600, color: T.white, fontSize: 6.5, letterSpacing: '.08em', marginTop: 1 }}>{label}</span>
      </>
    );
    const css: React.CSSProperties = {
      width: T.size, height: T.size, background: bg, borderRadius: '22%',
      display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
      paddingTop: 3, flexShrink: 0, border: 'none', cursor: 'pointer', textDecoration: 'none',
    };
    return to
      ? <Link to={to} style={css}>{inner}</Link>
      : <button onClick={onClick} style={css} title={label}>{inner}</button>;
  };
  ...
  return (
    <header style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 300, background: navBg, transition: 'background 0.4s ease' }}>
      ...
      <div style={{ ...containerStyle, display: 'flex', alignItems: 'center', gap: 14, padding: `10px ${T.paddingX}` }}>
        ...
        <Link to="/" style={{ display: 'flex', alignItems: 'center', gap: 11, flexShrink: 0, textDecoration: 'none' }}>
          <div style={{
            ...
            <span style={{ fontFamily: T.black, color: T.madre, fontSize: 26, lineHeight: 1 }}>M</span>
          </div>
          <div style={{ lineHeight: 1 }}>
            <div style={{ fontFamily: T.black, color: T.white, fontSize: 19, letterSpacing: '.02em' }}>MARKET</div>
        ...
        <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
          <Pill bg={T.second}  label="SECOND"  onClick={toggleMode} />
          <Pill bg={T.gourmet} label="GOURMET" to="/?gourmet=1" />
        </div>

        {/* Buscador */}
        <div style={{
          ...elStyle, flex: 1, minWidth: 0,
          background: searchFocused ? '#3A567C' : T.navyField,
          borderRadius: 9, transition: 'background 200ms ease', padding: '0 16px', gap: 12,
        }}>
          <input
            type="text"
          ...
          />
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={T.placeholder} strokeWidth="2.2" strokeLinecap="round">
        ...
        {currentUser ? (
          <Link
            ...
            style={{
              ...elStyle, justifyContent: 'center', gap: 6, background: T.madre, borderRadius: 9, border: 'none',
            ...
            onMouseEnter={e => (e.currentTarget.style.background = T.madreHover)}
            onMouseLeave={e => (e.currentTarget.style.background = T.madre)}
          >
          ...
          <button
            ...
            style={{
              ...elStyle, justifyContent: 'center', background: T.madre, borderRadius: 9, border: 'none',
        ...
        <div onClick={onCartClick} style={{ ...elStyle, cursor: 'pointer', position: 'relative', flexShrink: 0 }}>
          ...
          {cartCount > 0 && (
            <span style={{ position: 'absolute', top: -4, right: -6, fontSize: 11, background: T.second, color: T.white, fontWeight: 700, lineHeight: 1, zIndex: 10, borderRadius: 999, padding: '1px 5px' }}>
              {cartCount}
            </span>
          )}
        </div>
      ...
      {/* ── DIVISOR (verde Second) ────────────────────── */}
      <div style={{ ...containerStyle, padding: `0 ${T.paddingX}` }}>
        <div style={{ height: 2, background: T.second }} />
      </div>
